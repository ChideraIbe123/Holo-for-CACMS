#!/bin/bash
# Controller-ranking benchmark: run each controller closed-loop in the sim, record
...
run_one () {
  ...
  $PY mavros_bridge.py --headless --control --duration $DUR > $OUT/${tag}_bridge.log 2>&1 &
  local BR=$!
